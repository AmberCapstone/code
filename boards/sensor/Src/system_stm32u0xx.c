...
/* Software workaround added to keep Debug enabled after Boot_Lock activation
 * and RDP=1  */
#ifdef ENABLE_DBG_SWEN
    ...
    if (((tmp_seccr & FLASH_SECR_BOOT_LOCK) == FLASH_SECR_BOOT_LOCK) &&
        ...
        FLASH->ACR |= FLASH_ACR_DBG_SWEN; /* Debug access software enabled to
                                             avoid the chip to be locked when
                                             RDP=1 and Boot_Lock=1        */
    }
#endif /* ENABLE_DBG_SWEN */
}
 ...
 * factors.
 *
 *         (*) MSI_VALUE is a constant defined in stm32u0xx_hal.h file (default
 * value 4 MHz) but the real value may vary depending on the variations in
 * voltage and temperature.
 *
 *         (**) HSI_VALUE is a constant defined in stm32u0xx_hal.h file (default
 * value 16 MHz) but the real value may vary depending on the variations in
 * voltage and temperature.
 *
 *         (***) HSE_VALUE is a constant defined in stm32u0xx_hal.h file
...
void SystemCoreClockUpdate(void) {
    ...
    switch (RCC->CFGR & RCC_CFGR_SWS) {
        ...
        case (RCC_CFGR_SWS_1 |
              RCC_CFGR_SWS_0): /* PLL used as system clock  source */
            ...
            switch (pllsource) {
                ...
                case (RCC_PLLCFGR_PLLSRC_1 |
                      RCC_PLLCFGR_PLLSRC_0): /* HSE used as PLL clock source */
