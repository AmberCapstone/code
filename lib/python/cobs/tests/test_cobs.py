import cobs


def test_zero_string():
    decoded = bytes([0x00])
    encoded = bytes([0x01, 0x01, 0x00])

    encoded_actual = cobs.Encode(decoded)

    assert len(encoded_actual) == len(encoded)
    for b_expected, b_actual in zip(encoded, encoded_actual):
        assert b_expected == b_actual

    decoder = cobs.Decoder()
    assert decoder.Decode(encoded)
    assert decoder.length == len(decoded)
    for b_expected, b_actual in zip(decoded, decoder.output):
        assert b_expected == b_actual


def test_short():
    decoded = bytes([0x11, 0x22, 0x00, 0x33])
    encoded = bytes([0x03, 0x11, 0x22, 0x02, 0x33, 0x00])

    encoded_actual = cobs.Encode(decoded)

    assert len(encoded_actual) == len(encoded)
    for b_expected, b_actual in zip(encoded, encoded_actual):
        assert b_expected == b_actual

    decoder = cobs.Decoder()
    assert decoder.Decode(encoded)
    assert decoder.length == len(decoded)
    for b_expected, b_actual in zip(decoded, decoder.output):
        assert b_expected == b_actual


def test_ff_block():
    # fmt: off
    decoded = bytes([
        ])

    encoded = bytes([
        ])
    # fmt: on

    encoded_actual = cobs.Encode(decoded)

    assert len(encoded_actual) == len(encoded)
    for b_expected, b_actual in zip(encoded, encoded_actual):
        assert b_expected == b_actual

    decoder = cobs.Decoder()
    assert decoder.Decode(encoded)
    assert decoder.length == len(decoded)
    for b_expected, b_actual in zip(decoded, decoder.output):
        assert b_expected == b_actual

    # test partial decoding
    decoder = cobs.Decoder()
    last_decoded = 0
    for i in range(0, len(encoded) - 50, 10):
        last_decoded = i + 10
        assert not decoder.Decode(encoded[i:last_decoded])

    assert decoder.Decode(encoded[last_decoded:])
    assert decoder.length == len(decoded)
    for b_expected, b_actual in zip(decoded, decoder.output):
        assert b_expected == b_actual
